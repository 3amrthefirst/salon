...
class EServiceAPIController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return $this->sendResponse(['services' => $eServices], 'E Services retrieved successfully');
    }

    public function search(Request $request): JsonResponse
    {

        $salons = Salon::where(function ($q) use ($request) {
            if ($request->name) {
                $q->where('name','LIKE','%'. $request->name .'%');
            }
            if ($request->phone_number) {
                $q->where('phone_number','LIKE','%'. $request->phone_number .'%');
                $q->orWhere('mobile_number','LIKE','%'. $request->phone_number .'%');
            }
        })->latest()->get();

        return $this->sendResponse(['salons' => $salons ], 'Salons retrieved successfully');
    }
